...
export function LoginForm({
  ...
  // Get last auth method from localStorage on component mount
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const stored = localStorage.getItem('lastAuthMethod');
      if (stored && ['email', 'google', 'github', 'discord'].includes(stored)) {
        setLastAuthMethod(stored as 'email' | 'google' | 'github' | 'discord');
      }
    }
  }, []);
  ...
  return (
    <div className={cn('flex flex-col gap-6', className)} {...props}>
      <Card>
        <CardHeader>
          <CardTitle>
            {showSignIn ? 'Login to your account' : 'Create an account'}
          </CardTitle>
          <CardDescription>
            {showSignIn
                ...
                <div className="flex items-center justify-between">
                  ...
                  {lastAuthMethod && (
                    <div className="flex items-center gap-2">
                      ...
                      <AuthMethodBadge authMethod={lastAuthMethod} />
                    </div>
                  )}
                </div>
